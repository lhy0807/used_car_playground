...
def ny_demo():
    ...
    usa = gpd.read_file('map/zipcodes.shp')
    ...
    data = pd.DataFrame(Record.objects.all().values(), dtype=str)
    ...
    for _,car in data.iterrows():
        avg_price[car['zip_code']][car['model']] += int(car['price'].replace(',', ''))
        counter[car['zip_code']][car['model']] += 1
